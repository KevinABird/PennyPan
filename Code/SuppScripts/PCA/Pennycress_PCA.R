...
pos_list<-read.gdsn(index.gdsn(genofile, "snp.position"))

chrpos<-cbind(chr_list, pos_list)
chrpos<-as.data.table(chrpos)
chrpos$pos_list<-as.numeric(chrpos$pos_list)

# Plot positions
ggplot(chrpos, aes(x=pos_list)) + geom_histogram(binwidth = 100000)+facet_wrap(~chr_list, scales="free_x")+theme_light()
...
pdf("SNPRelate_PCAs_20250902.pdf")
ggplot(tab, aes(x=EV1, y=EV2, colour = as.factor(is.ref), shape=as.factor(is.ref))) + 
  ...
  geom_point()+
  theme_light()+
  xlab(paste("PC 1 (",round(pc.percent[1],2),"%)",sep=""))+
...
ggplot(tab, aes(x=EV2, y=EV3, colour = as.factor(is.ref), shape=as.factor(is.ref))) + 
  ...
  geom_point()+
  theme_light()+
  xlab(paste("PC 2 (",round(pc.percent[2],2),"%)",sep="")) +
  ylab(paste("PC 3 (",round(pc.percent[3],2),"%)",sep=""))  +
  theme(legend.position="none") +
  geom_label_repel(label=tab$plot_label) +
...
ggplot(tab.chr6, aes(x=EV1, y=EV2, colour = as.factor(is.ref.Armenia), shape=as.factor(is.ref))) + 
  ...
  geom_point()+
  theme_light()+
  xlab(paste("PC 1 (",round(pc.percent.chr6[1],2),"%)",sep=""))+
  ylab(paste("PC 2 (",round(pc.percent.chr6[2],2),"%)",sep="")) +
  theme(legend.position="none") +
